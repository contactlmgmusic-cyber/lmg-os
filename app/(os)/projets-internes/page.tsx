...
export default async function InternalProjectsPage() {
  ...
  return (
    <main className="p-6 text-white md:p-10">
      <div className="mb-10 flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between"><div><p className="mb-3 text-xs font-semibold uppercase tracking-[0.3em] text-yellow-400">Organisation LMG</p><h1 className="text-4xl font-bold md:text-5xl">Projets internes</h1><p className="mt-3 text-zinc-400">{profile.role === ROLES.ARTISTIC_DIRECTOR ? "Uniquement les projets internes auxquels tu participes." : "Centralisez les briefs, consignes, décisions et ressources de l’équipe."}</p></div>{[ROLES.SUPER_ADMIN, ROLES.ADMIN].includes(profile.role) && <Link href="/projets-internes/nouveau" className="rounded-xl bg-white px-5 py-3 font-medium text-black">+ Nouveau projet interne</Link>}</div>
