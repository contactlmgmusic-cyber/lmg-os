...
export default async function CrmPage() {
  ...
  return <main className="min-h-screen bg-black px-5 py-8 text-white md:px-10"><div className="mx-auto max-w-[1500px]">
    <header className="flex flex-col gap-6 border-b border-zinc-900 pb-8 xl:flex-row xl:items-end xl:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.25em] text-yellow-500">Développement LMG</p><h1 className="mt-3 text-4xl font-bold md:text-6xl">Cockpit CRM</h1><p className="mt-3 max-w-3xl text-zinc-500">Les opportunités, contacts et relances de LMG dans une seule vue de pilotage.</p></div><div className="flex flex-wrap gap-3"><Link href="/prospects/nouveau" className="rounded-xl bg-white px-5 py-3 text-sm font-bold text-black">+ Nouveau prospect</Link><Link href="/booking/nouveau" className="rounded-xl border border-zinc-700 px-5 py-3 text-sm font-bold text-zinc-300">+ Nouveau booking</Link></div></header>
    ...
    <section className="mt-8 rounded-[26px] border border-zinc-800 bg-zinc-950 p-5 md:p-7"><div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-yellow-500">Discipline CRM</p><h2 className="mt-2 text-2xl font-bold">Aucune opportunité sans prochaine action</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-zinc-500">Chaque contact actif doit avoir un statut clair, un responsable et une date de relance. Les Kanban spécialisés restent l’espace de traitement détaillé.</p></div><Link href="/dashboard/priorites" className="shrink-0 text-sm font-semibold text-zinc-400 hover:text-white">Voir les priorités stratégiques →</Link></div></section>
  </div></main>;
}
