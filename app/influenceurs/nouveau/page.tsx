...
export default function NouveauInfluenceurPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error } = await supabaseBrowser.from("influenceurs").insert({
      nom,
      plateforme,
      pseudo,
      email,
  ...
  return (
    <main className="min-h-screen bg-black p-10 text-white">
      <div className="mb-10">
        <h1 className="text-5xl font-bold">Nouvel influenceur</h1>

        <p className="mt-3 text-zinc-400">
          Ajouter un créateur TikTok, Instagram, YouTube ou Twitch.
        </p>
      </div>
      ...
      >
        ...
        >
          {loading ? "Création..." : "Créer l'influenceur"}
        </button>
      </form>
    </main>
  );
