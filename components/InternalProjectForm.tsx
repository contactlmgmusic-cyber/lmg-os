...
export default function InternalProjectForm({ profiles, project }: { profiles: Profile[]; project?: Project }) {
  ...
  return (
    <form onSubmit={submit} className="space-y-6 rounded-3xl border border-zinc-800 bg-zinc-900 p-6 xl:p-8">
      ...
      <div className="grid gap-5 xl:grid-cols-2"><label className="space-y-2"><span className="text-sm text-zinc-400">Décisions et arbitrages</span><textarea value={form.decisions} onChange={(e) => update("decisions", e.target.value)} className={`${fieldClass} min-h-36`} placeholder="Décisions prises et raisons..." /></label><label className="space-y-2"><span className="text-sm text-zinc-400">Ressources et liens</span><textarea value={form.ressources} onChange={(e) => update("ressources", e.target.value)} className={`${fieldClass} min-h-36`} placeholder="Liens Drive, documents, références, contacts..." /></label></div>
